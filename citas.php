...
?>
  ...
  <link rel="stylesheet" type="text/css" href="../css/estilo5.css">
    ...
    <body>
        ...
        <form method="post">
          ...
          <input type="date" class="form-control text-uppercase" name="Fecha" placeholder="MM/DD/AAAA"
          required value="<?php if(isset($_GET['editar'])) echo $getROW['Fecha'];  ?>" />
            <!-- Campo de entrada de hora -->
            Selecciona la hora deseada:
            <input type="time" class="form-control text-uppercase" name="Hora" placeholder="00/00Am/Pm"
            required value="<?php if(isset($_GET['editar'])) echo $getROW['Hora'];  ?>" />
            <input type="text" class="form-control text-uppercase" name="Tipotrabajo" placeholder="Tipotrabajo"
          required value="<?php if(isset($_GET['editar'])) echo $getROW['Tipotrabajo'];  ?>"  
          </div>
 ...
 <table class="table table-hover table-bordered shadow p-3 mb-5 bg-danger rounded ">
    ...
    <?php
     $res = $MySQLiconn->query("SELECT * FROM citas");
    ...
    ?>
       <tr>
         ...
         <td>
           <a href="?editar=<?php echo $row['Idcitas']; ?>" onclick="return confirm('¿Deseas Editarlo ?'); ">

             <span class="glyphicon glyphicon-pencil"></span>

           </a>

           <a href="?eliminar=<?php echo $row['Idcitas']; ?>" onclick="return confirm('¿Seguro deseas eliminarlo?'); ">
             <span class="glyphicon glyphicon-trash"></span>
           </a>

           <a href="?imprimir=<?php echo $row['Idcitas']; ?>" onclick="return confirm('¿Deseas Imprimir ?'); ">
